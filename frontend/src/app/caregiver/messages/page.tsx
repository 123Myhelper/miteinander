...
function CaregiverMessagesContent() {
  ...
  useEffect(() => {
    const cleanup = onNewMessage((message: Message) => {
      if (activeConversation && message.conversationId === activeConversation.id) {
        ...
        if (message.senderRole !== 'care_giver') {
          markAsRead(activeConversation.id);
        }
      }
      fetchConversations();
    ...
    const cleanup = onMessagesRead((data) => {
      if (activeConversation && data.conversationId === activeConversation.id) {
        setMessages(prev => prev.map(m =>
          m.senderRole === 'care_giver' ? { ...m, isRead: true } : m
        ));
      }
    });
    return cleanup;
  ...
  const handleSendSettlement = async () => {
    if (!activeConversation || isSendingSettlement) return;
    setIsSendingSettlement(true);
    await sendSettlementRequest(activeConversation.id);
                  ...
                  groupedMessages.map((group, gi) => (
                    <div key={gi}>
                      ...
                      {group.messages.map((msg) => {
                        const isMine = msg.senderRole === 'care_giver';
                        ...
                        // Settlement request message (sent by caregiver)
                        if (msg.messageType === 'settlement_request') {
                          return (
                            <div key={msg.id} className="flex justify-center mb-3">
                              <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-3 max-w-[85%]">
                                ...
                                <p className="text-xs text-amber-600">{t('settlement.requestSentInfo')}</p>
                                <span className="text-[10px] text-amber-400 mt-1 block">{formatTime(msg.createdAt)}</span>
                              </div>
                            </div>
                          );
                        }
